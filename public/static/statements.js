async function accountStatement() {
  ...
  root.innerHTML=`<div class="grid2"><div class="panel"><h2>Upload transaction log</h2><p>Upload the Cofundr Excel log to update the ledger and prepare your statement.</p><form id="pdfStatementForm"><label class="datepick">Transaction log (.xlsx, up to 10 MB)<input id="pdfLedgerFile" type="file" accept=".xlsx"></label><label class="datepick mt">Include transactions through<input id="pdfCutoff" type="date" value="${DATA.summary.last_transaction_date||''}"><small>Leave blank to use the latest transaction date.</small></label><button id="pdfPreviewButton" class="primary mt" type="submit">Prepare statement</button></form><p id="pdfStatementError" role="alert" class="danger"></p></div><div class="panel"><h2>Same MISB statement format</h2><p><b>Investor 5490 · Amanahraya Trustees Berhad</b></p><p>Cofundr letterhead, account summary and six-column transaction detail, using the supplied statement layout.</p><p>Includes all completed cash transactions from the beginning of the uploaded log through the cut-off date. Deposits and withdrawals use the approved entries and their Excel approval timestamps. Duplicate requests are excluded; withdrawal fees are retained separately.</p><p class="muted">Uploading here replaces the workspace transaction ledger. The previous workbook version remains saved in D1.</p></div></div><div id="pdfStatementPreview" class="panel mt"><p>Prepare the statement to review totals and download the PDF.</p></div>`;
